...
- **iam.py**: This file is for create the two associated roles: AmazonElasticMapReduceRole and AmazonElasticMapReduceforEC2Role.
- **s3.py**: Control and manage the initial configuration that our S3 bucket needs, scripts, logs, configuration files, etc..
- **poller.py**: This is checking a function for status each N seconds until reach a specified status.
- **emr.py**: This file contains the functions to create an emr cluster and add steps to the cluster using boto3.

#### Main process:
- **emr_process.py**: This file is wrapping the basic functions of the boto3 library, the difference is that it is modified to interpret and understand our configuration files or project template.
